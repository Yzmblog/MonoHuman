...
class Trainer(object):
    ...
    def get_loss(self, net_output, 
        ...
        loss_dict = {loss_names[i]: train_losses[i] for i in range(len(loss_names))}


        if net_output.__contains__('loss_consis'):
            loss_consis = net_output['loss_consis']
            if not torch.isnan(loss_consis):
                loss_dict.update(
                    {'loss_consis': loss_consis}
                )
                train_losses.append(loss_consis)

        return sum(train_losses), loss_dict
            ...
            if not is_reload_model:
                if self.iter % cfg.train.save_checkpt_interval  == 0:
                    ...
                    if cfg.ddp:
                        ...
                        self.save_ckpt('latest')
                if cfg.save_all:
                    if self.iter in [5000, 10000, 30000] or \
    ...
    def progress(self):
        ...
        for _, batch in enumerate(tqdm(self.prog_dataloader)):
            ...
            batch['iter_val'] = torch.full((1,), self.iter)
            data = cpu_data_to_gpu(
                    batch, exclude_keys=EXCLUDE_KEYS_TO_GPU + ['target_rgbs', 'in_vertex'])
            with torch.no_grad():
                net_output = self.network(**data)
        ...
        if cfg.ddp:
            Image.fromarray(tiled_image).save(
                os.path.join(cfg.logdir, "prog_{:06}_{:02}.jpg".format(self.iter, self.local_rank)))
        else:
            Image.fromarray(tiled_image).save(
