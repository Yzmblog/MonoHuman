        ...
        for key, val in cpu_data.items():
            ...
            if isinstance(val, list):
                ...
                if not isinstance(val[0], str): # ignore string instance
                    gpu_data[key] = [x.cuda().to(local_rank) for x in val]
            elif isinstance(val, dict):
                gpu_data[key] = {sub_k: sub_val.cuda().to(local_rank) for sub_k, sub_val in val.items()}
            else:
                gpu_data[key] = val.cuda().to(local_rank)
    else:
        gpu_data = {}
        for key, val in cpu_data.items():
            # if key == 'in_dst_Rs':
