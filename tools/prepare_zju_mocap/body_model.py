...
import os
from tools.prepare_zju_mocap.lbs import batch_rodrigues, lbs
...
class SMPLlayer(nn.Module):
    ...
    def __init__(self,
        ...
        model_paths = {
            'male': os.path.join(model_path, MALE_PATH),
            'female': os.path.join(model_path, FEMALE_PATH),
            'neutral': os.path.join(model_path, NEUTRAL_PATH)
        }
        
        if osp.isdir(model_path):
            # model_fn = 'SMPL_{}.{ext}'.format(gender.upper(), ext='pkl')
            model_fn = model_paths[gender]
            smpl_path = model_fn
        else:
    ...
    def forward(self,
        ...
        return vertices, joints, joints_transform, bones_transform, self.faces
